...
{
    ImGuiContext& g = *GImGui;
    PushStyleColor(ImGuiCol_Text, ImColorToImVec4(text_color[1]));
    if (fmt[0] == '%' && fmt[1] == 's' && fmt[2] == 0)
        TextEx(va_arg(args, const char*), NULL, ImGuiTextFlags_NoWidthForLargeClippedText); // Skip formatting
    else
        TextV(fmt, args);
...
bool ImGui::Product(const char* label, ImTextureID texture_id, int number, const char* subscribe, const char* last_update, const char* last_launch)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(pos, pos + ImVec2(700, 170));

    const ImRect button_bb(bb.Min + ImVec2(370, 110), bb.Min + ImVec2(370, 110) + ImVec2(310, 40));
    ItemSize(ImVec2(700, 170), style.FramePadding.y);
    ItemAdd(bb, id);

    const char* product_icons[4] = { "d", "g", "t", "e" };

    bool hovered, held;
    bool pressed = ButtonBehavior(button_bb, id, &hovered, &held, 0);

    it_anim->second.stroke_color = ImLerp(it_anim->second.stroke_color, hovered ? ImColor(1.f, 1.f, 1.f, 0.2f) : ImColor(1.f, 1.f, 1.f, 0.f), anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, background_color, style.FrameRounding);

    window->DrawList->AddRectFilled(button_bb.Min, button_bb.Max, main_color, style.FrameRounding);

    window->DrawList->AddText(center_text(ImVec2(button_bb.Min.x - icon_width / 2, button_bb.Min.y), button_bb.Max, "Load"), text_color[0], "Load");

    PushFont(icon_small);
    icon_width = CalcTextSize("l").x;
    window->DrawList->AddText(ImVec2(center_text(ImVec2(button_bb.Min.x - icon_width / 2, button_bb.Min.y), button_bb.Max, "Load").x + CalcTextSize("Load").x - CalcTextSize("l").x / 4, center_text(button_bb.Min, button_bb.Max, "l").y), text_color[0], "l");
    PopFont();

    window->DrawList->AddImageRounded(texture_id, bb.Min, bb.Min + ImVec2(350, 170), ImVec2(0, 0), ImVec2(1, 1), ImColor(1.f, 1.f, 1.f, 1.f), style.FrameRounding, ImDrawFlags_RoundCornersLeft);

    PushFont(big_icon);
    window->DrawList->AddText(center_text(bb.Min, bb.Min + ImVec2(350, 170), product_icons[number]) - ImVec2(0, 15), text_color[0], product_icons[number]);

    PopFont();

    PushFont(medium_icon_font);
    window->DrawList->AddText(bb.Min + ImVec2(383, 22) - ImVec2(7, 6), main_color, "5"); // ImVec2(7, 6) its offset if you want to change icon size :)
    window->DrawList->AddText(bb.Min + ImVec2(383, 66) - ImVec2(7, 6), main_color, "4");
    window->DrawList->AddText(bb.Min + ImVec2(558, 22) - ImVec2(7, 6), main_color, "3");
    PopFont();

    window->DrawList->AddText(bb.Min + ImVec2(410, 15), text_color[0], "Subscription till:");
    window->DrawList->AddText(bb.Min + ImVec2(410, 59), text_color[0], "Last launch:");
    window->DrawList->AddText(bb.Min + ImVec2(584, 15), text_color[0], "Last update:");

    window->DrawList->AddText(bb.Min + ImVec2(410, 32), main_color, subscribe);
    window->DrawList->AddText(bb.Min + ImVec2(410, 76), main_color, last_launch);
    window->DrawList->AddText(bb.Min + ImVec2(584, 32), main_color, last_update);

    window->DrawList->AddText(center_text(bb.Min, bb.Min + ImVec2(350, 170), label) + ImVec2(0, 15), text_color[0], label);

    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Min + ImVec2(350, 170), ImColor(background_color.Value.x, background_color.Value.y, background_color.Value.z, 0.f), background_color, background_color, ImColor(background_color.Value.x, background_color.Value.y, background_color.Value.z, 0.f));
...
struct button_state {
    ...
    ImVec4 particle_color[50];

    bool particle_down[50];
};

bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const char* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PushFont(icon_small);
    const float icon_width = CalcTextSize(icon).x;
    PopFont();
    ...
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, hovered ? main_color : background_color, anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), style.FrameRounding);

    window->DrawList->AddText(center_text(ImVec2(bb.Min.x - icon_width / 2, bb.Min.y), bb.Max, label), text_color[0], label);

    for (int i = 0; i < 50; i++)
    {
        if (pressed)
        {
            it_anim->second.particle_pos[i].x = bb.Min.x + 1 + rand() % (int)size.x;
            it_anim->second.particle_pos[i].y = bb.Min.y + 1 + rand() % (int)size.y;

            it_anim->second.particle_speed[i].x = -1 + rand() % 4;
            it_anim->second.particle_speed[i].y = 1 + rand() % 3;
        }

        it_anim->second.particle_pos[i] += it_anim->second.particle_speed[i];

        it_anim->second.particle_color[i] = ImLerp(it_anim->second.particle_color[i], ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, (it_anim->second.particle_pos[i].y - bb.Max.y)), anim_speed);

        window->DrawList->AddCircleFilled(it_anim->second.particle_pos[i], 1.4f, GetColorU32(it_anim->second.particle_color[i]), 30);
    }

    PushFont(icon_small);
    window->DrawList->AddText(ImVec2(center_text(ImVec2(bb.Min.x - icon_width / 2, bb.Min.y), bb.Max, label).x + CalcTextSize(label).x - CalcTextSize(icon).x / 4, center_text(bb.Min, bb.Max, icon).y), text_color[0],  icon);
    PopFont();
...
bool ImGui::IconButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const char* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, icon_button_state() });
        it_anim = anim.find(id);
    }
    ...
    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, hovered ? text_color[0] : main_color, anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), style.FrameRounding);

    window->DrawList->AddText(ImVec2(bb.Min.x + (label_width + icon_width) / 2, center_text(bb.Min, bb.Max, label).y), text_color[0], label);

    PushFont(big_icon);
    window->DrawList->AddShadowCircle(ImVec2(bb.Min.x + (label_width + icon_width) / 2 - label_width / 2 + icon_width / 2, center_text(bb.Min, bb.Max, icon).y + CalcTextSize(icon).y / 2), big_icon->FontSize / 2, GetColorU32(it_anim->second.icon_color), 30.f, ImVec2(0, 0));
    window->DrawList->AddText(ImVec2(bb.Min.x + (label_width + icon_width) / 2 - label_width / 2, center_text(bb.Min, bb.Max, icon).y), GetColorU32(it_anim->second.icon_color), icon);
    PopFont();

    return pressed;
}

bool ImGui::Button(const char* label, const ImVec2& size_arg, const char* icon)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, icon);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = ImRect(bb_frame.Min + ImVec2(30, -19), bb_frame.Max - ImVec2(30, 23));
    ...
    // Render
    const ImU32 bg_col = ImColor(226, 153, 85, 25);
    const ImU32 grab_col = GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);
    window->DrawList->AddRectFilled(bb_frame.Min + ImVec2(30, -18), bb_frame.Max - ImVec2(30, 24), bg_col, style.FrameRounding, 0);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, main_color, style.FrameRounding);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    static std::map<ImGuiID, image_button_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, image_button_state() });
        it_anim = anim.find(id);
    }
    ...
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, hovered ? main_color : background_color, anim_speed);
    it_anim->second.image_offset = ImLerp(it_anim->second.image_offset, hovered ? ImVec2(5, 5) : ImVec2(0, 0), anim_speed);

    it_anim->second.stroke_color = ImLerp(it_anim->second.stroke_color, hovered ? ImColor(1.f, 1.f, 1.f, 0.2f) : ImColor(1.f, 1.f, 1.f, 0.f), anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), style.FrameRounding);

    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.stroke_color), style.FrameRounding, 0, 2.f);

    window->DrawList->AddImageRounded(texture_id, bb.Min + ImVec2(15, 15), bb.Max - ImVec2(15, 15), ImVec2(0,0), ImVec2(1,1 ), ImColor(1.f, 1.f, 1.f, 1.f), style.FrameRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(1, 1));
    ImGui::TextColored(ImVec4(0.839f, 0.839f, 0.839f, 1.f), label);
    ImGui::Spacing();
    PopStyleVar();
    ...
    const ImVec2 frame_size = CalcItemSize(size_arg - ImVec2(50, 0), CalcItemWidth(), (is_multiline ? default_font->FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(50, 0), window->DC.CursorPos + ImVec2(50, 0) + frame_size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Min + frame_size);
    ...
    {
        PushFont(dot_font);
        const ImFontGlyph* glyph = g.Font->FindGlyph('p');
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (default_font->FontSize * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax((int)((inner_size.y - style.FramePadding.y) / default_font->FontSize), 1);
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline) { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - default_font->FontSize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline) { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + default_font->FontSize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline) { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * default_font->FontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline) { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * default_font->FontSize; }
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background_color), style.FrameRounding, ImDrawFlags_RoundCornersRight);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y);
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(0, 10) + style.FramePadding;
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            cursor_offset.y = searches_result_line_no[0] * default_font->FontSize;
            if (searches_result_line_no[1] >= 0)
            {
                select_start_offset.x = InputTextCalcTextSizeW(&g, ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x;
                select_start_offset.y = searches_result_line_no[1] * default_font->FontSize;
            }

            // Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1224)
            if (is_multiline)
                text_size = ImVec2(inner_size.x, line_count * default_font->FontSize);
        }
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - default_font->FontSize < scroll_y)
                    scroll_y = ImMax(0.0f, cursor_offset.y - default_font->FontSize);
        ...
        if (render_selection)
        {
            ...
            {
                if (rect_pos.y > clip_rect.w + default_font->FontSize)
                    break;
                ...
                {
                    it_anim->second.rect_size = ImLerp(it_anim->second.rect_size, InputTextCalcTextSizeW(&g, p, text_selected_end, &p, NULL, true), anim_speed * 4);
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - default_font->FontSize), rect_pos + ImVec2(it_anim->second.rect_size.x, bg_offy_dn));
                ...
                rect_pos.y += default_font->FontSize;
            }
        }

        ImVec2 buf_size = CalcTextSize(buf);

        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length) {

            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x - draw_scroll.x, center_text(frame_bb.Min, frame_bb.Max, buf_display).y), GetColorU32(it_anim->second.text_color), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - default_font->FontSize + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
            ...
            if (!is_readonly)
            {
                ...
                g.PlatformImeData.InputLineHeight = default_font->FontSize;
            }
        }
    }
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length) {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x, center_text(frame_bb.Min, frame_bb.Max, buf_display).y), GetColorU32(it_anim->second.text_color), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(50, 0), frame_bb.Min + ImVec2(0, frame_bb.GetSize().y), GetColorU32(it_anim->second.background_color), style.FrameRounding, ImDrawFlags_RoundCornersLeft);

    PushFont(icon_font);
    draw_window->DrawList->AddShadowCircle(frame_bb.Min + ImVec2(-25, 25), 10.f, GetColorU32(it_anim->second.main_color), 40.f, ImVec2(0,0));
    draw_window->DrawList->AddText(center_text(frame_bb.Min - ImVec2(50, 0), frame_bb.Min + ImVec2(0, frame_bb.GetSize().y) + ImVec2(2, 0), hint), GetColorU32(it_anim->second.main_color), hint);
    PopFont();
